"""Generate new Pokemon images from a trained checkpoint (uses EMA generator)."""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Pokemon images (v2)")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to model checkpoint")
    parser.add_argument("--num", type=int, default=16, help="Number of images to generate")
    parser.add_argument("--output", type=str, default=None, help="Output image path")
    parser.add_argument("--seed", type=int, default=None, help="Random seed")
    parser.add_argument("--no-ema", action="store_true", help="Use raw generator instead of EMA")
    ...
    if args.output is None:
        ...
        args.output = f"generated_{epoch_str}_{model_type}_wgan_num{args.num}.png"

    # Load generator
    generator = Generator().to(device)
    checkpoint = torch.load(args.checkpoint, map_location=device, weights_only=False)

    if not args.no_ema and "ema_generator_state_dict" in checkpoint:
        generator.load_state_dict(checkpoint["ema_generator_state_dict"])
        print("Loaded EMA generator weights")
    else:
        generator.load_state_dict(checkpoint["generator_state_dict"])
        print("Loaded raw generator weights")
    generator.eval()
